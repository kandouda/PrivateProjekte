...
public class UploadDownloadFileServlet extends HttpServlet {
	...
	public void init() throws ServletException {
		// Wenn die Datei klein ist dann wird sie in Memory geladen amsonsten
		// auf Disk
		DiskFileItemFactory fileFactory = new DiskFileItemFactory();
		File filesDir = (File) getServletContext().getAttribute(
				"FILES_DIR_FILE");
		// die hochgeladene Dateien sollen in diesem Folder gespeichert als tmp
		// files
		...
		try {
			...
			while (fileItemsIterator.hasNext()) {
				...
				if (fileItem.getSize() > 0) {
					...
					File file = new File(getServletContext().getAttribute(
							"FILES_DIR")
							+ File.separator + fileItem.getName());

					if (file.exists()) {
						file.delete();
						file = new File(getServletContext().getAttribute(
								"FILES_DIR")
								+ File.separator + fileItem.getName());
					}
					...
					fileItem.write(file);
					out.write("<br>* File mit diesem Name "
							+ fileItem.getName() + " uploaded successfully.");
					out.write("<br><a href=\"updownload.do?fileName="
